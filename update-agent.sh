...
if : > "${LOCK_FILE}" 2>/dev/null && exec 9>>"${LOCK_FILE}"; then
    if command -v flock >/dev/null 2>&1 && ! flock -n 9; then
        # Запрос убирается, хотя выполнен не будет. Оставленный, он дождался
        # бы конца текущего прогона и запустил бы нас снова: дорожка следит
        # за появлением файла, а не за тем, кто его положил. После УДАЧНОГО
        # обновления это безвредно — версия уже стоит, и мы ответим «не
        # новее». А после неудачного с откатом версия прежняя, запрос всё
        # ещё новее, и сервер повторил бы неудавшееся обновление сам, никого
        # не спросив (amnezia-vpn-server-nukf).
        #
        # Внятность ответа обеспечивает состояние: панель читает его и видит
        # идущее обновление, а не тишину.
        rm -f "${REQUEST_FILE}"
