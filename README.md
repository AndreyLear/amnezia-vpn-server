...
**Повторите те же флаги, что при установке** — прежде всего домены. Мастер
берёт адрес, который получат новые настройки клиентов, из флагов запуска: без
`--vpn-domain` он запишет туда IP сервера. Уже выданные настройки от этого не
портятся, но новые будут выданы с другим адресом.
