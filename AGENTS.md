...
anything contradictory in the managed blocks above.
...
- **Accepted changes are recorded in CHANGELOG.md** by the orchestrator
  (local file, intentionally gitignored — do not add it to git).
- Changelog entries follow Keep a Changelog, [Unreleased] section,
  Russian, one line per change with the task ID.
