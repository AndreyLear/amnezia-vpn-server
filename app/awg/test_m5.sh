...
# Четвёртым полем идёт возраст рукопожатия: по одним счётчикам обрыв
# туннеля и плеер, добирающий буфер, выглядят одинаково — нулями
# (amnezia-vpn-server-3wbe). У peer B рукопожатие было (возраст — любое
# число секунд, он считается от «сейчас»), у A и Z — никогда, и это не
# ноль, а прочерк: «сию секунду» и «никогда» не одно и то же.
check "история: счётчики совпадают со status.json" \
    grep -Eq "AAAA-peer-a-:10:20:- BBBB-peer-b-:100:200:[0-9]+ ZZZZ-peer-z-:0:0:-" "${SPEED_S}"
check "история: возраст рукопожатия дописан" \
    grep -Eq " BBBB-peer-b-:100:200:[0-9]+" "${SPEED_S}"
check "история: «рукопожатия не было» — не ноль" \
    not grep -q "AAAA-peer-a-:10:20:0 " "${SPEED_S}"
