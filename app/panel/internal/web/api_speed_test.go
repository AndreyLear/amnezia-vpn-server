...
func TestAPIClientSpeed10MinWindowSpansTenMinutes(t *testing.T) {
	...
	//
	// Считаем не число заполненных столбцов, а ГДЕ они: скорость
	// принадлежит отрезку и красит все столбцы, которых он касается
	// (amnezia-vpn-server-bwp3), поэтому свежая пара даёт больше одного
	// столбца — и это правильно. Врало бы другое: данные в начале окна,
	// куда двадцатиминутной давности замер попасть не должен.
	...
	for i, v := range down {
		...
		filled++
		if i < len(down)/2 {
			t.Fatalf("столбец %d из %d заполнен: замер старше окна просочился в ответ", i, len(down))
		}
	}
	if filled == 0 {
		t.Fatalf("свежий замер в ответ не попал: %v", down)
	}
}
