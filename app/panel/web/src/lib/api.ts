...
export type Client = {
  ...
  mtu: number;
  /**
   * Предел скорости к клиенту в мегабитах; 0 — без предела. Режет скорость до
   * заданной и взамен убирает повторные передачи и рывки — размен, а не
   * ускорение (amnezia-vpn-server-jzzu, amnezia-vpn-server-ouhb).
   */
  rate_limit: number;
  /**
