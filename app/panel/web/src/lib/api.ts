...
export type Client = {
  ...
  mtu: number;
  /**
   * Наибольший MTU, который можно задать клиенту на этом сервере: потолок
   * интерфейса с учётом добивки S4 (amnezia-vpn-server-bctr). 0 или нет —
   * неизвестно, действует общая граница 1440.
   */
  mtu_max?: number;
  /**
   * Предел скорости к клиенту в мегабитах; 0 — без предела. Держит поток
   * ровным: без рывков и повторных передач (amnezia-vpn-server-jzzu).
   */
  rate_limit: number;
  /**
...
export type UpdateInfo = {
  ...
  dismissed: string;
  /**
   * Время итога, который человеку уже показали. Пока оно не совпадает с
   * state_at_utc, итог показывается — в том числе после перезапуска панели,
   * который делает само обновление.
   */
  outcome_seen: string;
};
