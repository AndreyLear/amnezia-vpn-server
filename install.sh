...
tunnel_mtu_preflight() {
    ...
    # Здесь только путь минус 60 байт: добивку S4 установщик ещё не знает
    # (параметры появляются при server init), её вычитает панель при каждой
    # генерации awg0.conf (awgconf.TunnelMTUs, amnezia-vpn-server-bctr).
...
render_nftables() {
    ...
    if [ "$dns_intercept" = "1" ]; then
        ...
        # that does gets none of the tunnel's DNS — no AAAA filter, no
        # protection from a tampered answer — while believing otherwise,
        # because the config came from us. Anything entering the tunnel
        # for port 53 lands on our resolver instead.
        #
        # redirect rather than dnat: it translates to the address of the
        # incoming interface, which is awg0's own, so the rule survives a
        # different --vpn-subnet without repeating the address here.
        #
        # iifname "awg0" is the whole safety of it. Without that match the
        # same rule would answer the WAN and make the box an open resolver.
        prerouting="$(cat <<'PREROUTING'

    chain prerouting {
        type nat hook prerouting priority dstnat; policy accept;
        iifname "awg0" udp dport 53 redirect to :53
        iifname "awg0" tcp dport 53 redirect to :53
    }
PREROUTING
        )"
    fi
...
table ip amnezia {
    ...
    set dns_seen {
        type ipv4_addr
        # 10 минут, а не полчаса. Клиент, который чем-то пользуется,
        # спрашивает имена постоянно: записи живут минуты. С получасовым
        # окном роутер владельца, отправляющий всё в DoH, попадал в
        # множество из-за одного обращения после перезапуска туннеля — и
        # панель показывала «всё в порядке» ровно для того случая, ради
        # которого отметка и делалась.
        timeout 10m
    }
