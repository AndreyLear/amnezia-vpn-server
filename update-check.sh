...
# Пункт «Проверить обновления» в панели (amnezia-vpn-server-8bt5). Панель
# наружу не ходит и ходить не должна — она кладёт пустой файл в свой том
# данных, а дорожка systemd будит эту же проверку. Файл убирается сразу:
# дорожка следит за его появлением, и оставленный запрос звал бы нас снова.
REQUEST_FILE="${AMNEZIA_UPDATE_CHECK_REQUEST:-${AMNEZIA_UPDATE_ROOT:-/opt/amnezia-vpn}/data/update-check-request}"
rm -f "${REQUEST_FILE}"
